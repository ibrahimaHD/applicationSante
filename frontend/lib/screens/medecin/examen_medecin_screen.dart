          ...
          child: Container(
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                AppTextField(label: 'Type d\'examen *', hint: 'Ex: Bilan sanguin, Radio thorax...', prefixIcon: Icons.science_outlined, controller: typeCtrl),
                const SizedBox(height: 12),
                AppTextField(label: 'Résultat', hint: 'Résultat de l\'examen', prefixIcon: Icons.assignment_outlined, controller: resultatCtrl, maxLines: 3),
                const SizedBox(height: 12),
                ...
                Row(children: [
                  ...
                  ChoiceChip(label: const Text('Normal'), selected: statut == 'normal',
                      onSelected: (_) => setStateModal(() => statut = 'normal'),
                      selectedColor: AppColors.success.withOpacity(0.2)),
                  const SizedBox(width: 6),
                  ChoiceChip(label: const Text('Attention'), selected: statut == 'attention',
                      onSelected: (_) => setStateModal(() => statut = 'attention'),
                      selectedColor: Colors.orange.withOpacity(0.2)),
                  const SizedBox(width: 6),
                  ChoiceChip(label: const Text('Urgent'), selected: statut == 'urgent',
                  ...
                  onPressed: () async {
                    ...
                    final response = await http.post(
                      ...
                      body: jsonEncode({
                        ...
                        'type_examen': typeCtrl.text,
                        'resultat': resultatCtrl.text,
                        'statut': statut,
                        'date_examen': dateCtrl.text,
                      }),
                    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text('Résultats d\'examens', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w600)),
      ...
      body: _isLoading
          ...
          : Center(
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Container(
                  ...
                  child: Column(children: [
                    Container(
                      ...
                      child: const Icon(Icons.science_outlined, color: Color(0xFF3949AB), size: 40),
                    ),
                    const SizedBox(height: 16),
                    const Text('Ajouter un résultat d\'examen', style: AppTextStyles.heading2, textAlign: TextAlign.center),
                    const SizedBox(height: 8),
                    const Text(
                ...
                Text('${_patients.length} patient(s) disponible(s)', style: const TextStyle(color: AppColors.textSecondary)),
              ]),
            ),
    );
  }
